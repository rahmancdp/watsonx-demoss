...
llm = Model(model="bigcode/starcoder",credentials=creds, params=params)

def buildprompt(code,sourcelang,targetlang):
...
def codeconvert(incode,sourcelang,targetlang):
    ...
    code = ""
    for response in llm.generate_async(prompts,ordered=True):
        code += response.generated_text
    return code

temp_dir = tempfile.TemporaryDirectory()

st.header("code converter powered by watsonx")
